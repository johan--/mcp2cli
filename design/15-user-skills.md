...
> **MUST READ** [users/SKILL.md](users/SKILL.md) for custom workflows and tips. Not overwritten by updates.
```
...
| 命令 | 对 `users/` 的行为 |
|------|-------------------|
| `generate skill`（首次） | 创建 `users/` 目录 + `.gitkeep` + 空模板 `users/SKILL.md` |
| `generate skill`（增量/`--full`） | **不触碰** `users/` 目录 |
