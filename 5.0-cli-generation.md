...
默认使用 `claude -p` 调用 LLM，同时支持 Codex、Anthropic SDK、OpenAI 兼容接口等多种后端。不同后端在 Prompt 模式（文件引用 vs 内联）、文件访问能力、会话续接方式上有所不同。

配置文件位于 `~/.agents/mcp2cli/config.yaml`。

> 各后端的详细调用方式、配置格式和降级策略见 [5.2-llm-backend.md](5.2-llm-backend.md)。
...
└── generator/
    ...
    └── templates/
        ├── cli_gen_skill.md      # 生成规则和约束（详见 5.1-cli-gen-templates.md）
        ├── cli_gen_example.md    # 示例 YAML 输出（详见 5.1-cli-gen-templates.md）
